...
#include <utils/Log.h>
#include <pthread.h>
// ace - try "updated" header for consistency
...
static int processSolicited(RilClientPrv *prv, Parcel &p) {
    ...
    if (DBG) LOGD("%s()", __FUNCTION__);

    status = p.readInt32(&token);
    if (status != NO_ERROR) {
        LOGE("%s: Read token fail. Status %d\n", __FUNCTION__, status);
        return RIL_CLIENT_ERR_IO;
    }
    ...
    if (req_func)
    {
        ...
        if (DBG) LOGD("%s: No handler for token %d\n", __FUNCTION__, token);
    }
...
static int processRxBuffer(RilClientPrv *prv, void *buffer, size_t buflen) {
    ...
    if (DBG) LOGD("%s: status %d response_type %d", __FUNCTION__, status, response_type);
